    ...
    >
      <Container className="skills-page">
        ...
        <div className="skills-terminal">
          ...
          <RevealGroup className="capability-grid skills-capabilities skills-bento" mode="scroll" stagger={48}>
            {CAPABILITIES.map((item, index) => (
              <Reveal
                key={item.icon}
                type="card"
                className={`skills-bento__cell skills-bento__cell--${index + 1}`}
              >
                <article
                  className={`capability-card capability-card--${item.accent}${
                    index === 0 ? " capability-card--primary" : " capability-card--secondary"
                  }`}
                >
