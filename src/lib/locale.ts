...
export const LOCALE_BOOTSTRAP_SCRIPT = `(function(){var bot=/bot|crawl|spider|slurp|bingpreview|facebookexternalhit/i.test(navigator.userAgent||"");if(!bot)document.documentElement.classList.add("js-reveal");var k=${JSON.stringify(LOCALE_STORAGE_KEY)};var p=location.pathname;var isEs=p==="/es"||p.indexOf("/es/")===0;var current=isEs?"es":"en";var stored=null;try{stored=localStorage.getItem(k)}catch(e){}function toPath(l){if(l==="es")return p==="/"||p===""?"/es":isEs?p:"/es"+p;if(p==="/es"||p==="/es/")return "/";return isEs?(p.slice(3)||"/"):p}if(stored==="en"||stored==="es"){if(stored!==current){location.replace(toPath(stored)+location.search+location.hash);return}current=stored}else{if(!bot){var lang=(navigator.languages&&navigator.languages.length?navigator.languages[0]:navigator.language)||"";var code=String(lang).toLowerCase().replace("_","-");current=code==="es"||code.indexOf("es-")===0?"es":"en";if(current!==(isEs?"es":"en")){try{localStorage.setItem(k,current)}catch(e){}location.replace(toPath(current)+location.search+location.hash);return}}}try{localStorage.setItem(k,current)}catch(e){}document.documentElement.lang=current;if(p==="/"||p===""||p==="/es"||p==="/es/")document.documentElement.classList.add("home-nav-wait")})()`
